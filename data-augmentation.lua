...
function data_augmentation(x, y)
    ...
    if x:dim() == 4 then
        -- jitter for training
        local new_x = torch.Tensor(x:size(1) * scale * 2, 3, 24, 24)
        local new_y = torch.Tensor(y:size(1) * scale * 2, 1)
        -- Memory saving (my touch)
        --local new_x = x:resize(x:size(1) * scale * 2, 3, 24, 24)
        --local new_y = y:resize(y:size(1) * scale * 2, y:size(2))
        for i = 1, x:size(1) do
            local src = x[i]
            local images = {}
            ...
            for j = 1, #images do
                ...
                new_y[scale * 2 * (i - 1) + j] = y[i] --:copy(y[i])
                new_x[scale * 2 * (i - 1) + #images + j]:copy(horizontal_reflection(images[j]))
                new_y[scale * 2 * (i - 1) + #images + j] = y[i]--:copy(y[i])
            end
            if i % 100 == 0 then
                collectgarbage()
            end
